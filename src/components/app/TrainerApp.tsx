...
export interface ReviewEntry extends DecisionScore {
  street: Street;
  pot: number;
  equity: number;
...
export interface TrainerSeed {
  ...
  hand?: HandState | null;
  entries?: ReviewEntry[];
  session?: CompletedHand[];
  actedOn?: Street | null;
}

/**
 * The Trainer tab. `seed` pre-populates state so a design-capture route can
 * render a specific view; production passes nothing.
 */
export function TrainerApp({ seed }: { seed?: TrainerSeed } = {}) {
  const [config, setConfig] = useState<HandConfig>(seed?.config ?? {
    playerCount: TRAINER_PLAYER_COUNT,
    variant: 4,
    ante: 6,
    doubleBoard: true,
  });
  const [hand, setHand] = useState<HandState | null>(seed?.hand ?? null);
  const [entries, setEntries] = useState<ReviewEntry[]>(seed?.entries ?? []);
  const [session, setSession] = useState<CompletedHand[]>(seed?.session ?? []);
  /**
   ...
   */
  const [actedOn, setActedOn] = useState<Street | null>(seed?.actedOn ?? null);
